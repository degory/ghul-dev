...
A trailing `mut` makes the variable reassignable, and then the initializer can be dropped for a deferred-init local that starts at its type's default value: `let total mut = 0` reassigns later, and `let result: int mut;` defaults to 0. Either form can also take its value from `_`, which initializes to the default of the type the context expects - `let x = _`, or `_[T]` to pin the type. This is a different `_` from the discard placeholder below - where a value is expected it means "default value", where `_` would name a variable it means "no name needed" - and the two never collide because they only ever appear in different positions.
