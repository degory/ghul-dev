...
`T?` over a value type - a scalar such as `int`, or a struct - is backed by .NET's `Nullable<T>` at the IL level. You don't need to use `Nullable<T>` directly: write `T?`, the same way you would for a reference type. A ghūl `int?` is a `Nullable<int>` as far as the runtime is concerned, so it passes to and from non-ghūl .NET code as it is:
