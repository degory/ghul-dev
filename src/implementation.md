...
tool, with source in the [`ghul-lang/ghul`](https://github.com/ghul-lang/ghul)
repository.
...
| `synthesize-iterator-reset`{:text}   | Gives a type implementing `Collections.Iterator[T]` that declares no `reset` one that throws `System.NotSupportedException`. |
