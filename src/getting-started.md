...
The quickest way to run ghūl locally needs no project at all. With the .NET SDK installed, the `ghul`{:sh} command runs a single `.ghul`{:text} file directly and starts an interactive session, as [scripts, the REPL and notebooks](/scripts-and-repl) describes:
