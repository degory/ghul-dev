...
A few rules keep a wrapped expression unambiguous. A line opening with `.`, `|` or `|>` carries the expression above it on, which is how member chains and pipes wrap. Four other tokens could have carried it on and deliberately do not: a line-start `(`, `[`, operator or `rec` opens a new statement rather than continuing the line above as a call, an index, an infix operand or a rec-lambda marker. So a wrapped operator expression puts the operator at the end of the line rather than the start of the next, and a `rec(n - 1)` self-call can open a line inside a recursive lambda. Every other token starts a new statement because nothing joins it to the line above in the first place. A bare `return` at the end of a line is a void return when the next line opens with a closing keyword, and takes the next line's expression as its value otherwise; the two readings never compete, because a statement written after a `return` in the same block would be unreachable. In a parenthesised group, a top-level `,` commits the tuple reading and a line break commits the [block reading](/expression-oriented-programming.html#blocks), exactly as a written `;` does.
