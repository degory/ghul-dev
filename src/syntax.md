...
A line break ends a construct that is complete and carries one that is not. `1 +` at the end of a line continues onto the next because the expression is unfinished, and so does an open `(` still waiting for its arguments. Most line-start tokens need no rule beyond that: a `let`, an identifier or a literal cannot continue the line above, so there is nothing to decide.

Two sets are worth naming. A line opening with `.`, `|` or `|>` continues the construct above even though it is complete, which is how member chains and pipes wrap. A line opening with `(`, `[`, an operator or `rec` does not, though each of them could: the construct above is complete, so the break ends it rather than reading a call, an index, an infix operand or a rec-lambda marker. That is why a wrapped operator expression puts the operator at the end of the line rather than the start of the next, and why `rec(n - 1)` can open a line inside a recursive lambda.

A bare `return` at the end of a line is a void return when the next line opens with a closing keyword, and takes the next line's expression as its value otherwise; the two readings never compete, because a statement written after a `return` in the same block would be unreachable. In a parenthesised group, a top-level `,` commits the tuple reading and a line break commits the [block reading](/expression-oriented-programming.html#blocks), exactly as a written `;` does.
