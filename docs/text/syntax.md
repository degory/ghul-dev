...
A line break ends a statement only where the statement is complete. An expression left unfinished at the end of a line, such as `a +` or an open `(`, continues on the next line, and a line beginning with `.` or `|>` continues the line above, so member chains and pipes wrap in the usual way. For code formatted in the conventional way these rules give the reading you would expect. The full set is under [statement terminators](https://ghul.dev/grammar.html#statement-terminators) in the grammar, for the occasions when a wrapped expression does not parse the way it reads.
